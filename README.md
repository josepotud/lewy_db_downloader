...
3. **Explorar y Analizar**:
   ...
   - **📋 Explorador**: Vista tabular con apertura directa de fichas clínicas al pulsar sobre el ID y diagnósticos truncados con información al pasar el ratón (*hover*).
   - **🔍 Búsqueda Libre**: Búsqueda global instantánea (<15ms) en todos los campos, etiquetas y valores.
   - **⏱️ Fusión Temporal**: Emparejamiento multimodal de pruebas dentro de ventanas de tolerancia configurables ($\pm	ext{días}$) con 1 sola fila por paciente y aislamiento estricto.
...
- **HTML5 / CSS3 Moderno** (Diseño clínico profesional, responsivo y accesible).
- **JavaScript ES6+ Vanilla** (Motor de análisis, indexación en memoria y cálculos clínicos).
- **SheetJS (xlsx.full.min.js)** (Lectura y exportación de libros Excel multi-pestaña).
- **PapaParse** (Procesamiento ultra-rápido de CSV y exportaciones delimitadas).
- **Chart.js** (Visualizaciones clínicas y gráficos interactivos).
